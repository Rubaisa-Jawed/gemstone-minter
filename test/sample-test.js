...
describe("GemstoneMinter", function () {
  ...
  it("Should print 2 different URIs", async function () {
    ...
    const mintTx = await gemstoneMinter.whitelistMint(addr1.address, 1);
    await mintTx.wait();
    ...
    const mintTx2 = await gemstoneMinter.whitelistMint(addr1.address, 2);
    await mintTx2.wait();
  ...
  it("Should get purchases correctly", async function () {
    ...
    const mintTx2 = await gemstoneMinter.whitelistMint(addr2.address, 2);
    await mintTx2.wait();
